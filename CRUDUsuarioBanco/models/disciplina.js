...
module.exports = class Disciplina {
    ...
    static async listar() {
        ...
        await Sql.conectar(async (sql) => {
            lista = await sql.query("select id_disciplina, nome_disciplina, carga_horaria_disciplina, semestre_materia, ano_disciplina, presenca_aberta, p.id_professor, nome_professor, c.id_curso, nome_curso from disciplina as d inner join professor as p on p.id_professor = d.id_professor inner join curso as c on c.id_curso = d.id_curso order by nome_disciplina asc");
        });
        return (lista || []);
    }
